...
import { ReporteVolumenBuscarResponse } from 'src/app/models/response/reporteVolumenBuscarResponse';
...
import { BarChartReporteVolumenComponent } from '../../grafico-charts/bar-chart-reporte-volumen/bar-chart-reporte-volumen.component';
...
export class ReporteVolumenComponent implements OnInit {
  ...
  listReporteVolumenBuscarResponse: ReporteVolumenBuscarResponse[];
  itemsPagination: ReporteVolumenBuscarResponse[];
  ...
  getReporte(pagina: number) {
    ...
    const reporteVolumenConsultarRequest = {
    ...
    this.reporteService
      .listVolumen(reporteVolumenConsultarRequest, page)
      .subscribe(
        (response) => {
          if (response.estado == 'ERROR') {
            ...
            if (response.paginacion.totalRegistros > 0) {
              ...
              this.listReporteVolumenBuscarResponse = response.resultado;
        ...
        (error) => {
          this.blockUI.stop();
          this.listReporteVolumenBuscarResponse =
            new Array<ReporteVolumenBuscarResponse>();
          this.itemsPagination = [];
          this.total = 0;
        }
      );
  ...
  exportExcel() {
    let nameFile = 'REPORTE-VOLUMEN'+'_'+this.formattedDate(new Date());
    this.excelService.generateExcel(
      this.listReporteVolumenBuscarResponse,
      nameFile,
      'LISTA DE REPORTE VOLUMEN',
      'rv'
    );
  ...
  exportPDF(): void {
    const parametros: {
      ...
      lstReporte: this.listReporteVolumenBuscarResponse,
      //nCoddat: this.codeDatalogger,
    };

    this.reporteService.obtenerVolumenPDF(parametros).subscribe(
      (data: any) => {
        let file = new Blob([data], { type: 'application/pdf' });
        const fileURL = URL.createObjectURL(file);
        var a = document.createElement('a');
        a.href = fileURL;
        a.download = 'REPORTE-VOLUMEN'+'_'+this.formattedDate(new Date())+'.pdf';
  ...
  verGrafico() {
    const dialogRef = this.dialog.open(BarChartReporteVolumenComponent, {
      height: this.height,
      width: this.width,
      data: { reporte: this.listReporteVolumenBuscarResponse },
    });
  ...
  getReportePaginada(): void {
    this.itemsPagination = [];
    let j = 0;
    if (this.listReporteVolumenBuscarResponse.length > 0) {
      if (
        this.listReporteVolumenBuscarResponse.length <
        this.page * this.limit
      ) {
        for (
          let i = (this.page - 1) * this.limit;
          i < this.listReporteVolumenBuscarResponse.length;
          i++
        ) {
          this.itemsPagination[j] = this.listReporteVolumenBuscarResponse[i];
          j++;
        }
      } else {
        for (
      ...
      this.itemsPagination = this.listReporteVolumenBuscarResponse;
    }
  }
