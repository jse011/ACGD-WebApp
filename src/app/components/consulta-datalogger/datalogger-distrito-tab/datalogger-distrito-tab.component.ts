...
import { DataloggersPorDistritoBuscarResponse } from 'src/app/models/response/dataloggersPorDistritoBuscarResponse';
import { DataloggerService } from 'src/app/_service/datalogger.service';
import { ExcelService } from 'src/app/_service/excel.service';
import { SessionService } from 'src/app/services';
import { BarChartDataloggerPorDistritoComponent } from '../../grafico-charts/bar-chart-datalogger-por-distrito/bar-chart-datalogger-por-distrito.component';
import { MatDialog } from '@angular/material/dialog';
import { ChartDataset, ChartOptions, ChartType } from 'chart.js';
@Component({
  selector: 'app-datalogger-distrito-tab',
  templateUrl: './datalogger-distrito-tab.component.html',
  styleUrls: ['./datalogger-distrito-tab.component.scss'],
})
export class DataloggerDistritoTabComponent implements OnInit {
  ...
  codeDatalogger: any;
  listDataloggersBuscarResponse: DataloggersPorDistritoBuscarResponse[];
  itemsPagination: DataloggersPorDistritoBuscarResponse[];
  ...
  ngOnInit(): void {
    ...
    this.listDataloggersBuscarResponse =
      new Array<DataloggersPorDistritoBuscarResponse>();
  ...
  getDataloggers(pagina: number) {
    ...
    this.dataloggerService.listDataloggersPorDistrito(page).subscribe(
      (response) => {
        if (response.estado == 'ERROR') {
          ...
          if (response.paginacion.totalRegistros > 0) {
            ...
            this.toastr.info('No se encontraron dataloggers por distrito', 'Información', {
              closeButton: true,
            });
          }
          this.blockUI.stop();
  ...
  exportExcel() {
    let nameFile = 'DATALOGGER-POR-DISTRITO'+'_'+this.formattedDate(new Date());
    this.excelService.generateExcel(
      this.listDataloggersBuscarResponse,
      nameFile,
      'LISTA DE DATALOGGER POR DISTRITO',
      'dd'
    );
  ...
  exportPDF(): void {
    ...
    this.dataloggerService
      .obtenerDataloggersPorDistritoPDF(parametros)
      .subscribe(
        (data: any) => {
          ...
          a.href = fileURL;
          a.download = 'DATALOGGER-POR-DISTRITO'+'_'+this.formattedDate(new Date())+'.pdf';
  ...
  verGrafico() {
    const dialogRef = this.dialog.open(BarChartDataloggerPorDistritoComponent, {
      height: this.height,
      width: this.width,
